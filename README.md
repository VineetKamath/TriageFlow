...
2. Scoring layer: the backend scores each zone every cycle and classifies it as green, amber, or red. Live rainfall (Open‑Meteo) applies a pressure multiplier.
...
5. Presentation layer: the frontend shows ward polygons, incidents, unit movement, suggestions, optimization, forecasts, and impact analytics in real time.

## Key endpoints (REST)
- `GET /state`: current zones/units/suggestions/incidents
- `GET /weather`: Bengaluru rainfall (mm/hr) + updated_at
- `GET /optimize`: resource allocation plan + optimization score
- `GET /predict/{zone_id}`: 60‑minute pressure forecast + surge flag
- `GET /impact`: minutes saved / lives impacted / CO₂ avoided
- `POST /mutual-aid`: request mutual aid (creates a synthetic incident)
...
- F: run flood scenario
- R: reset simulation
- A: accept first pending suggestion
- C: toggle analysis panel
- O: toggle optimization panel
- Esc: close overlays/panels

## Windows note
`start.sh`, `demo.sh`, and `check.sh` are bash scripts. On Windows, run the backend and frontend in separate terminals:
...
## Demo
Run the flood scenario and watch **Bellandur (Zone 7)** move from early stress to critical red while adjacent zones lose capacity. You can see resource depletion, volunteer drop, and incident reports unfold in a deterministic sequence. This demonstrates that TriageFlow does not just visualize events — it predicts pressure and recommends response actions before full collapse.
