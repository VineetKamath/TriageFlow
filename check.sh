...
echo "Waiting for Bellandur (Zone 7) to hit red (max 240s)..."
ZONE_RED=0
for _ in $(seq 1 80); do
  STATE_JSON=$(curl -s --max-time 10 "${BASE_URL}/state")
  if [[ -n "${STATE_JSON}" ]]; then
    CHECK_RESULT=$(python -c "import json,sys; data=json.loads(sys.argv[1]); z=next((x for x in data.get('zones',[]) if x.get('id')==7),{}); print('RED' if z.get('status')=='red' and z.get('name')=='Bellandur' else 'WAIT')" "${STATE_JSON}" 2>/dev/null)
...
if [[ ${ZONE_RED} -ne 1 ]]; then
  record_failure "Zone 7 did not reach red within 4 minutes as Bellandur"
fi
